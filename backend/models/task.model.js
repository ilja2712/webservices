...
const Task = function(task) {
  ...
  Task.create = (user_id, newTask, result) => {
    sql.query(`insert into task 
      ...
      console.log("Задача создана", { id: res.title, ...newTask });
      result(null, { id: res.title, ...newTask });
    });
  };
  ...
  Task.updateByIdTaskState = (user_id, task_id, task, result) => {
    sql.query(`UPDATE task 
        ...
        console.log("Обновлен статус: ", { task_id: task_id, ...task_id });
...
Task.updateByIdTaskAll = (task_id, task, result) => {
  sql.query(`update task 
              set Name = '${task.title}',
                Description = '${task.description}',
                Date_task = STR_TO_DATE('${task.date_task}', '%d/%m/%Y'),
                ID_PRIORITY = (select p.ID_PRIORITY from priority p
                        where p.Name = '${task.priority}')
      ...
      console.log("Задача изменена", { id: res.title, ...task });
      result(null, { id: res.title, ...task });
      }
  );
};
...
Task.removeWS = (id, result) => {
  sql.query(`DELETE FROM TASK WHERE ID_STATE = ${Number(id)}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
